...
let private parseSort (sort: string option) =
    match sort with
...
let private readDetail (r: NpgsqlDataReader) : Dtos.AssetDto =
    { Id = r.GetGuid 0
      ...
      Condition = (if r.IsDBNull 10 then None else Some(Pg.AssetCondition.ofDb (r.GetString 10)))
...
let search (ds: NpgsqlDataSource) (scope: DataScope) (f: SearchFilters) : Task<Dtos.PageDto<Dtos.AssetListItemDto>> =
    task {
        ...
        use countCmd = new NpgsqlCommand($"SELECT count(*) FROM asset a WHERE {where}", conn)
        ...
        while more do
            ...
            if hasRow then items.Add(readListItem reader) else more <- false
...
let tryGetById (ds: NpgsqlDataSource) (scope: DataScope) (id: Guid) : Task<Dtos.AssetDto option> =
    task {
        ...
        use conn = ds.CreateConnection()
        do! conn.OpenAsync()
        use cmd = new NpgsqlCommand($"SELECT {detailColumns} FROM asset a WHERE a.id = @id AND {scopeSql}", conn)
...
let private optToStr (v: string option) = v
...
let private diff (before: Dtos.AssetDto) (after: Asset) : (string * string option * string option) list =
    ...
    [ "assetTag", Some before.AssetTag, Some after.AssetTag
      ...
      "osName", optToStr before.OsName, optToStr after.OsName
      "osVersion", optToStr before.OsVersion, optToStr after.OsVersion
      "macAddresses",
      (if before.MacAddresses.IsEmpty then None else Some(macs before.MacAddresses)),
      (if after.MacAddresses.IsEmpty then None else Some(macs after.MacAddresses))
      "condition", str before.Condition (sprintf "%A"), str after.Condition (sprintf "%A")
      "assignedTo", optToStr before.AssignedTo, optToStr after.AssignedTo
      "location", optToStr before.Location, optToStr after.Location
      ...
      "notes", optToStr before.Notes, optToStr after.Notes ]
...
let update
    ...
    task {
        ...
        use readCmd =
            new NpgsqlCommand($"SELECT {detailColumns} FROM asset a WHERE a.id = @id AND {scopeSql} FOR UPDATE", conn, tx)
...
let changeStatus
    ...
    task {
        ...
        use readCmd =
            new NpgsqlCommand($"SELECT a.status::text FROM asset a WHERE a.id = @id AND {scopeSql} FOR UPDATE", conn, tx)
        ...
        | current ->
            ...
            if not (AssetStatus.canTransition from target) then
                ...
                do! insertHistory conn tx id actor.UserId "status" (Some(Pg.AssetStatus.toDb from)) (Some(Pg.AssetStatus.toDb target))
