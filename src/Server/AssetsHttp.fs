...
let private problem (ctx: HttpContext) (status: int) (title: string) (detail: string option) (errors: Validation.FieldError list) : Dtos.ProblemDetailsDto =
    { Type = "about:blank"
      Title = title
      Status = status
      Detail = detail
...
let private problemJson (status: int) (title: string) (detail: string option) (errors: Validation.FieldError list) : HttpHandler =
    fun next ctx ->
...
let private writeError: AssetsDb.WriteError -> HttpHandler =
    ...
    | AssetsDb.DuplicateTag ->
        problemJson 409 "Conflict" (Some "asset tag already exists") [ { Field = "assetTag"; Message = "already exists" } ]
    | AssetsDb.DuplicateSerial ->
        problemJson
            409
            "Conflict"
            (Some "serial number already exists for this make")
            [ { Field = "serialNumber"; Message = "already exists for this make" } ]
    ...
    | AssetsDb.IllegalTransition(from, to') ->
        validationProblem [ { Field = "status"; Message = $"cannot change from %A{from} to %A{to'}" } ]
...
let private canWriteToUnit (scope: DataScope) (ds: NpgsqlDataSource) (unitId: Guid) =
    task {
        ...
        | SquadronScope(SquadronId s) ->
            ...
            do! conn.OpenAsync()
            use cmd = new NpgsqlCommand("SELECT EXISTS(SELECT 1 FROM unit WHERE id=@u AND squadron_id=@s)", conn)
...
let private qsGuid ctx name = qs ctx name |> Option.bind (fun s ->
...
let checkDuplicate (ds: NpgsqlDataSource) : HttpHandler =
    requireAuth (fun _ ->
        fun next ctx ->
            task {
                let! result =
                    AssetsDb.checkDuplicate ds (qs ctx "assetTag") (qs ctx "make") (qs ctx "serialNumber") (qsGuid ctx "excludeId")
            ...
            withScope p (fun scope ->
                fun next ctx ->
                    task {
                        ...
                        | [], None ->
                            return!
                                validationProblem [ { Field = "updatedAt"; Message = "is required on update" } ] next ctx
                        | [], Some expected ->
                            let! allowed = canWriteToUnit scope ds dto.UnitId
